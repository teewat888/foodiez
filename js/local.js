...
const localFn = (() => {
  ...
  const fetchAddFav = (fav) => {
    ...
    return fetch(localURL + 'favorite',confObj)
    .then(resp => resp.json())
  }
  const fetchUserFav = (id,user) => {
    ...
    console.log(confObj);
    return fetch(localURL + `favorite/${id}`,confObj)
    .then(resp => resp.json())
  }
  ...
  const checkFavorite = (element,id,userId,userName,act) => {
    ...
    .then(data => { 
      ...
      if (favoriteRecipe !== undefined) {
        const userFavorite = favoriteRecipe.users.find(el => el.id === userId); //check whether current user already fav?
        console.log('use fav id? ',userFavorite);
        if (userFavorite !== undefined) {  //recipe fav existed and user already like this
          if (act === 'render') {
            fullHeart(element);
          } else {
            const users = favoriteRecipe.users.filter(el => el.id !== userId); 
            
            fetchUserFav(id,users).then(() => emptyHeart(element))
            .catch(e => {console.log(e);});
          }
        } else {  // user currently not fav
          if (act === 'render') {
            emptyHeart(element);
          } else {
            const users = favoriteRecipe.users.filter(el => el.id !== userId); 
            ...
            fetchUserFav(id,users).then(() => fullHeart(element))
            .catch(e => {console.log(e);});
          }
        }
      } else {
        ...
        fetchAddFav(fav).then(() => fullHeart(element))
        .catch(e => {console.log(e);});
      }
    }}).catch((e) => {
      console.log(e);
